...
if(isVar("PReason_spankzchoir_late")) {  sendDungeonMessage(" Not delivering your %ass% to the mistress that bought it via spankzchoir in time",2);}  
...
if(isVar("Preason_BadChores")) { sendDungeonMessage( random("Failure to complete chores in a timely manner","Unfinished chores","Failure to do chores","Poor attitudes regarding chores","Failed to complete chores.."),2);} 

if(isVar("Preason_BadCum")) { sendDungeonMessage( random("Unauthorized ejaculation","Cumming without permission"),2);} 

if(isVar("Preason_BadEdging") ){ sendDungeonMessage( random("Unauthorized edging","Edging against %DomHonorific% %DomName%'s wishes"," being unable to resist Edging your %cock%" ),2);} 
...
if(isVar("Preason_BadFullTime")) { sendDungeonMessage( random("Failed to fulfill full time duties","Laziness","Failure to meet demands for proper slavery"),2);} 
...
answer=createInput(10, "Soft","Medium","Hard", "Extreme");
