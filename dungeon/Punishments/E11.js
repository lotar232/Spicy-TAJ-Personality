...
sendMessage(random("Lets see what we can do about you %Grin%","Its time to make you repent your sins","I suppose we need to correct your recent behaviour","I have something for you..")); //#DT4
    ...
    else
    {
     ...
     answer1 = sendInput("is it charged up slave??? ");
...
errors=0;
correct=0;
total_lines=randomInteger(18,30);
errors_allowed=randomInteger(3,5);

sendMessage ("ok %SubName%, you're going to write the following line "+ total_lines + " times");

sentence = replaceVocab(sentence);
sentencelength = Math.floor(sentence.length / 3);
...
sendMessage("%SlaveName%, every error or slow response will earn you a "+ random("little... ummmm .... motivation %Smile%","nice zap", "painful correction", "well deserved punishment"));
pishock(0,20,2,0);
setVar("shocks_recieved", 0);
...
{ response = createInput(sentencelength); 
	...
	if(response.timeout)
			...
			if (errors <= errors_allowed)
				{setTempVar("pishockintensity",random(50,100) );
				 ...
				 pishock(0,getVar("pishockintensity"),getVar("pishockduration"),0);
					...
					sendMessage(random("too slow!!","type faster bitch!","ugh...I'm waiting %SlaveName%")+" you still have "+(total_lines-correct) + " lines left" );}
			else {setTempVar("pishockintensity",60 );
		...
		if(response.contains(sentence))
			...
			else {setTempVar("pishockintensity",100 );
			setTempVar("pishockduration", 4);
			pishock(0,getVar("pishockintensity"),getVar("pishockduration"),0);
			sendMessage("wrong! Loser!");
			}
		}
	}
}
...
if (correct >= total_lines) {
	...
	sendMessage("I love how literate my %Slave%'s are");
	    setTempVar("E11Complete", true);
	setVar("PunishmentComplete", true);
	setVar("punishmentCompleted", 411);
